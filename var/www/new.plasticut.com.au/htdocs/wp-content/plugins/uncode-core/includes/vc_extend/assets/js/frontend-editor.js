...
window.vc || (window.vc = {}),
	function() {
		...
		vc.template = function(text, argument) {
			...
			var render, template = RegExp([(argument.escape || noMatch).source, (argument.interpolate || noMatch).source, (argument.evaluate || noMatch).source].join("|") + "|$", "g"),
				index = 0,
				source = "__p+='";
			text.replace(template, function(match, escape, interpolate, evaluate, offset) {
	...
	function($) {
		...
		void 0 === window.vc && (window.vc = {}), window.vc.ShortcodesBuilder = function(models) {
			...
			save: function(status) {
				...
				data.vc_post_custom_css = window.vc.$custom_css.val(), data.dynamic_css_cleanup = window.uncodeFrontEditorCleanupNonce.nonce, data.content = this.wpautop(string), status && (data.post_status = status, $(".vc_button_save_draft").hide(100), $("#vc_button-update").text(window.i18nLocale.update_all)), window.vc.update_title && (data.post_title = this.getTitle()), this.ajax(data).done(function() {
	...
	function($) {
		...
		window.vc.PresetSettingsUIPanelFrontendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperAjax).vcExtendUI({
			...
			createPreset: function(column_params) {
				...
				_.isUndefined(vc.ShortcodesBuilder) || (this.builder = new vc.ShortcodesBuilder), preset = (model = $(column_params.currentTarget)).data("preset"), tag = model.data("tag"), column_params = {
					width: "1/1"
				}, model = {
					shortcode: "vc_row",
					params: {}
					...
					shortcode: "vc_column",
					params: column_params,
					parent_id: (model = vc.shortcodes.create(model)).id,
					root_id: model.id
				}, options = {
					shortcode: tag,
					parent_id: vc.shortcodes.create(columnOptions).id,
					root_id: (model = model).id
	...
	function($) {
		...
		window.vc.cloneMethod_vc_tab = function(data, model) {
			...
			updateIfExistTab: function(model) {
				...
				var $tab = this.tabsControls().find("[data-tab-o-id=" + model.getParam("tab_id") + "]"),
					$content = this.tabsControls().closest('.wootabs').find('.tab-content');

				var icon_position = model.getParam("icon_position"),
					icon =  model.getParam("icon") !== '' ? '<i class="' + model.getParam("icon") + ' icon-position-' + ( icon_position !== '' ? icon_position : 'left' ) + '"></i>' : '',
				...
				return !!$tab.length && ($tab.attr("aria-controls", "tab-" + model.getParam("tab_id")).find("a > span").attr("href", "#tab-" + model.getParam("tab_id")).text(model.getParam("title"), $tab.data("model", model).attr('data-m-id', model.get("id"))).prepend(icon_left).append(icon_right), !0)
				// END UNCODE EDIT
			},
			buildControlHtml: function(model) {
				...
				var $tab = $('<li data-tab-id="tab-' + model.getParam("tab_id") + '" data-tab-o-id="' + model.getParam("tab_id") + '" class="vc_tta-tab' + active + '"><a href="#tab-' + model.getParam("tab_id") + '" data-toggle="tab"><span></span></a></li>');
				// $tab.find("> a > span i.fa").remove();
			...
			removeTab: function(index) {
				...
				// START UNCODE EDIT
				// var $tab = this.tabsControls().find("[data-m-id=" + model.get("id") + "]"),
				var $tab = this.tabsControls().find("[data-tab-id=tab-" + index.getParam("tab_id") + "]"),
				// END UNCODE EDIT
					index = $tab.index();
				// START UNCODE EDIT
	...
	function($) {
		...
		$(function() {
			...
			}, window.vc.events.on("undoredo:add undoredo:undo undoredo:redo undoredo:lock undoredo:unlock", checkControls), $undoControl.on("click.vc-undo", function(newContent) {
