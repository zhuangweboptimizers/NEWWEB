...
function vc_loop_form_field( $settings, $value ) {
	// BEGIN UNCODE EDIT
	if ( function_exists( 'uncode_override_vc_loop_form_field' ) ) {
		return uncode_override_vc_loop_form_field( $settings, $value );
	}
	// END UNCODE EDIT
...
class VcLoopSettings {
	...
	public function parse_post_type( $value ) {
		...
		// START UNCODE EDIT
		$options = apply_filters( 'uncode_cpts_build_query_options', $options );
		// END UNCODE EDIT
