...
if ( class_exists( 'WooCommerce' ) && $dynamic === 'add-to-cart' && is_a( $product_object, 'WC_Product' ) ) {
	...
	echo '<span class="' . esc_attr(trim(implode(' ', $wrapper_class))) . '" '.implode(' ', $div_data_attributes).'><a href="' . $a_href . '" class="custom-link ' . esc_attr(trim($css_class)) . '"' . $title . $target . $onclick . $rel . $lightbox_data . $width . $el_id . '>' . $span_before . $bigtext_start . do_shortcode( apply_filters( 'uncode_get_vc_button_content', $content ) ) . $bigtext_end . $span_after . '</a>' . uncode_print_dynamic_colors_inline_style($inline_style_css) . '</span>';
}
