...
if ( 'yes' === get_option( 'woocommerce_enable_reviews', 'yes' ) ) {
	...
	$output = '<div class="uncode_woocommerce_widget uncode_woocommerce_widget--top-rated-products wpb_content_element' . esc_attr( $el_class ) . '" ' . $el_id . '>';
	$type = 'WC_Widget_Top_Rated_Products';
	global $wp_widget_factory;
	// to avoid unwanted warnings let's check before using widget
	if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
		ob_start();
		$args = $use_widget_style === 'yes' ? uncode_get_default_widget_args( 'top_rated_products' ) : array();
