...
<!-- WISHLIST TABLE -->
<table
	class="shop_table cart wishlist_table wishlist_view traditional <?php echo uncode_switch_stock_string( $no_interactions ? 'no-interactions' : '' ); ?> <?php echo uncode_switch_stock_string( $enable_drag_n_drop ? 'sortable' : '' ); ?> "
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'yith-woocommerce-wishlist' ) ) ); ?>"><i class="fa fa-cross"></i></a>
							</div>
						</td>
					<?php endif; ?>
					...
					<td class="product-thumbnail">
						...
						<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
							<?php echo uncode_switch_stock_string( $product->get_image() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</a>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'woocommerce' ); ?>">
							...
							<?php
							if ( $show_price ) {
								echo uncode_switch_stock_string( $item->get_formatted_product_price() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}

							if ( $show_price_variations ) {
								echo uncode_switch_stock_string( $item->get_price_variation() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}
							?>
					...
					<?php if ( $show_quantity ) : ?>
						<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'woocommerce' ); ?>">
							...
							<?php if ( ! $no_interactions && $wishlist->current_user_can( 'update_quantity' ) ) : ?>
								<div class="quantity">
									<input class="input-text qty text" type="number" min="1" step="1" name="items[<?php echo esc_attr( $item->get_product_id() ); ?>][quantity]" value="<?php echo esc_attr( $item->get_quantity() ); ?>"/>
								</div>
							<?php else : ?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart" data-title="<?php esc_attr_e( 'Actions', 'uncode' ); ?>">
	...
	?>
		<tr class="pagination-row wishlist-pagination">
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo uncode_switch_stock_string( $page_links ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
		</tr>
	<?php endif ?>
	</tbody>
