	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'activated_successfully'          => _x( 'The following plugin was activated successfully:', 'tgmap_activated_successfully_string', 'tgmpa' ),
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				...
				if ( ! $this->is_plugin_installed( $slug ) ) {
					if ( current_user_can( 'install_plugins' ) ) {
						...
						//Edited by UNCODE
						if ( true === $plugin['required'] && 'uncode-core' != $slug ) {
		...
		public function get_installed_version( $slug ) {
			...
			if ( isset( $this->plugins[ $slug ] ) && ! empty( $installed_plugins[ $this->plugins[ $slug ]['file_path'] ]['Version'] ) ) {
		...
		public function does_plugin_require_update( $slug ) {
			...
			$minimum_version   = isset( $this->plugins[ $slug ] ) && isset( $this->plugins[ $slug ]['version'] ) ? $this->plugins[ $slug ]['version'] : false;
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					// EDITED by UNCODE
					'<p>' . __( 'Installed version:', 'tgmpa' ) . '<span>%2$s</span></p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					// EDITED by UNCODE
					'<p>' . __( 'Minimum required version:', 'tgmpa' ) . '<span>%1$s</span></p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					// EDITED by UNCODE
					'<p>' . __( 'Available version:', 'tgmpa' ) . '<span>%2$s</span></p>',
					$color,
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				// EDITED by UNCODE
				// Remove h2 and inline styles of .update-php
